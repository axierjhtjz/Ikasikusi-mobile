...
namespace cocos2d
{
	...
	bool CCScrollLayer::initWithLayers(CCArray* layers, int widthOffset)
	{
		...
		m_tPagesIndicatorPosition = ccp(0.5f * getContentSize().width, ceilf(getContentSize().height / 8.0f));
	...
	void CCScrollLayer::updatePages()
	{
		...
		CCARRAY_FOREACH(m_pLayers, object)
		{
			...
			//layer->setPosition(ccp((i * (getContentSize().width - m_fPagesWidthOffset)), 0));
            layer->setPosition(ccp(0, (i * (getContentSize().height - m_fPagesWidthOffset))));
			if (!layer->getParent())
				addChild(layer);
			i++;
		}
	...
	{
		float pageFloat = - getPosition().y / (getContentSize().height - m_fPagesWidthOffset);
	...
	{
		//return ccp(pageNumber * -1.f * (getContentSize().width - m_fPagesWidthOffset), 0.0f);
        return ccp(0.0f, pageNumber * -1.f * (m_fPagesWidthOffset + getContentSize().height));
	}
	...
	void CCScrollLayer::ccTouchMoved(CCTouch* pTouch, CCEvent* pEvent)
	{
		...
		if (m_iState == kCCScrollLayerStateSliding)
		{
			float desiredY = (m_uCurrentScreen * -1.f * (getContentSize().height - m_fPagesWidthOffset)) + touchPoint.y - m_fStartSwipe;
	...
	void CCScrollLayer::ccTouchEnded(CCTouch* pTouch, CCEvent* pEvent)
	{
		...
		{
			selectedPage = pageNumberForPosition(getPosition());
